...
Имеется возможность указывать аннотации как для создаваемого метода с телом лямбда выражения, так и для параметров лямбда выражения, в случае указания аннотаций, параметры нужно обязательно обрамлять `()`, так что в начале за кавычками следуют аннотации метода, а внутри кавычек аннотации параметров:

```bsl

"&АннотацияМетода (Первый, Второй) -> Возврат Первый + Второй"

// Тут будет создан метод аннотированный `&АннотацияМетода`


"(Первый, &АннотацияПараметра Второй) -> Возврат Первый + Второй"

// Тут в методе будет аннотирован второй параметр аннотацией `&АннотацияПараметра`
